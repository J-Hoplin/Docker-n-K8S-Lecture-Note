...
도커란 **리눅스 기반의 컨테이너**에서 여러 기능을 추함으로써 애플리케이션 단위를 컨테이너로서 좀 더 쉽게 사용할 수 있도록 만들어놓은 오픈소스 프로젝트이다. `GO`언어로 작성되어있으며, 기존 가상화 기술은 `가상 머신`과는 다르게 성능에 손실이 매우 적다. 그렇기에 차세대 기술인 클라우드, 인프라 솔루션에서 필수적인 기술이다.
<br>
...
베이스이미지는 `Dockerfile`을 정의하여 만들 수 있다. 매번 이미지를 만들떄 이미지가 중복으로 생성되어 용량에 부담이 갈 수 도 있다 생각할 수 있지만, Docker는 베이스 이미지에서 바뀐 부분에 대해서만 이미지로 생성하고, 실행할때는 베이스이미지와 바뀐 부분을 합쳐서 실행한다.
